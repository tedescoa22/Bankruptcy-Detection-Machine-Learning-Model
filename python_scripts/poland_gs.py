...
gs_lr.fit(X_p_train_sc, y_p_train)
gs_rf.fit(X_p_train_sc, y_p_train)
gs_gbc.fit(X_p_train_sc, y_p_train)
gs_knn.fit(X_p_train_sc, y_p_train)
...
pred_lr = gs_lr.predict(X_p_test_sc, y_p_test)
pred_rf = gs_rf.predict(X_p_test_sc, y_p_test)
pred_gbc = gs_gbc.predict(X_p_test_sc, y_p_test)
pred_knn = gs_knn.predict(X_p_test_sc, y_p_test)
pred_ext = gs_ext.predict(X_p_test_sc, y_p_test)

df_preds_p = pd.DataFrame()
for func in func_list:
    preds = func.predict(X_t_train_sc, y_t_train)
    df_preds_p[f'{func}'] = df_preds_p

df_preds_p['actual_values'] = y_p_test
csv = df_preds_p.to_csv("df_preds_p.csv", index = False)
